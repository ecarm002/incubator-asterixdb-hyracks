...
public class VGroupTupleBufferManagerTest extends AbstractTupleMemoryManagerTest {

    VGroupTupleBufferManager bufferManager;
    final int PARTITION = 4;

    @Before
    public void setup() {
        bufferManager = new VGroupTupleBufferManager(Common.commonFrameManager, PARTITION,
                Common.BUDGET);
    }
    ...
    private void testCanNotInsertToAnyPartitions() throws HyracksDataException {
        for (int i = 0; i < PARTITION; i++) {
            assertFalse(bufferManager.insertTuple(i, tupleBuilder.getFieldEndOffsets(), tupleBuilder.getByteArray(), 0,
    ...
    private Map<TuplePointer, Integer> testInsertOnePartitionToFull(int idpart) throws HyracksDataException {
        ...
        for (int i = 0; i < inFTA.getTupleCount(); i++) {
            ...
            if (!bufferManager.insertTuple(idpart, tupleBuilder.getFieldEndOffsets(), tupleBuilder.getByteArray(), 0,
