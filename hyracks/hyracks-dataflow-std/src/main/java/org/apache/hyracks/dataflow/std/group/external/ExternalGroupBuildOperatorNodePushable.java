...
public class ExternalGroupBuildOperatorNodePushable extends AbstractUnaryInputSinkOperatorNodePushable
    ...
    private ExternalHashGroupBy externalGroup;
    private ExternalGroupState state;

    public ExternalGroupBuildOperatorNodePushable(IHyracksTaskContext ctx, Object stateId, int tableSize,
            int[] keyFields,
            int framesLimit,
    ...
    public void open() throws HyracksDataException {
        ...
        ISpillableTable table = spillableTableFactory.buildSpillableTable(ctx, tableSize, -1,
                keyFields, comparators,
                firstNormalizerComputer, aggregatorFactory, inRecordDescriptor, outRecordDescriptor, framesLimit, 0);
        RunFileWriter[] runFileWriters = new RunFileWriter[table.getNumPartitions()];
        this.externalGroup = new ExternalHashGroupBy(this, table, runFileWriters, inRecordDescriptor);

        state.setSpillableTable(table);
        state.setRuns(runFileWriters);
        state.setSpilledNumTuples(externalGroup.getSpilledNumTuples());
    }

    @Override
    public void nextFrame(ByteBuffer buffer) throws HyracksDataException {
        externalGroup.build(buffer);
    }
    ...
    public void close() throws HyracksDataException {
        externalGroup.flushSpilledPartitions();
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            ...
            LOGGER.fine("level 0:" + "build with " + numOfPartition + " partitions"
