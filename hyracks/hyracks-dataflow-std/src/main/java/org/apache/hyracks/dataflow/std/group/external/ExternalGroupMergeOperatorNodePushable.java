...
public class ExternalGroupMergeOperatorNodePushable extends AbstractUnaryOutputSourceOperatorNodePushable
    ...
    private static Logger LOGGER = Logger.getLogger("ExternalGroupbyMerge");
    ...
    public ExternalGroupMergeOperatorNodePushable(IHyracksTaskContext ctx, Object stateId,
    ...
    public void initialize() throws HyracksDataException {
        ...
        int[] sizeInTuples = aggState.getSpilledNumTuples();
        try {
            writer.open();
            doPass(table, partitionRuns, sizeInTuples, writer, 1); // level 0 use used at build stage.
        } catch (Exception e) {
            writer.fail();
    ...
    private void doPass(ISpillableTable table, RunFileWriter[] runs, int[] sizeInTuples, IFrameWriter writer, int level)
        ...
        for (int i = 0; i < runs.length; i++) {
            if (runs[i] != null) {
                ISpillableTable partitionTable = spillableTableFactory.buildSpillableTable(ctx, sizeInTuples[i],
                ...
                int[] sizeInTuplesNextLevel = buildGroup(runs[i].createReader(), partitionTable, runFileWriters);

                if (LOGGER.isLoggable(Level.FINE)) {
                    int numOfSpilledPart = 0;
                    for (int x = 0; x < sizeInTuples.length; x++) {
                        if (sizeInTuples[x] > 0) {
                            numOfSpilledPart++;
                        }
                    }
                    LOGGER.fine("level " + level + ":" + "build with " + sizeInTuples.length + " partitions"
