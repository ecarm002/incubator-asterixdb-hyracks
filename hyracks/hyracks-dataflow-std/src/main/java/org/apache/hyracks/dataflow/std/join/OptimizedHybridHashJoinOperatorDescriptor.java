...
public class OptimizedHybridHashJoinOperatorDescriptor extends AbstractOperatorDescriptor {
    ...
    public OptimizedHybridHashJoinOperatorDescriptor(IOperatorDescriptorRegistry spec, int memsize, int inputsize0,
        ...
        this(spec, memsize, inputsize0, factor, keys0, keys1, hashFunctionGeneratorFactories,
                    ...
                    if (!forceNLJ && (afterMax < (NLJ_SWITCH_THRESHOLD
                        ...
                        for (int rPid = rPStatus.nextSetBit(0); rPid >= 0; rPid = rPStatus.nextSetBit(rPid + 1)) {
                            ...
                            if (isLeftOuter || probeSideInTups < buildSideInTups) {
                                applyNestedLoopJoin(probeRd, buildRd, memsize, rprfw, rbrfw); //checked-modified
                            } else {
                                applyNestedLoopJoin(buildRd, probeRd, memsize, rbrfw, rprfw); //checked-modified
                            }
                        }
                    }
                }
