...
public class OptimizedHybridHashJoin {
    ...
    public void initBuild() throws HyracksDataException {
        bufferManager = new VGroupTupleBufferManager(ctx, numOfPartitions, memForJoin * ctx.getInitialFrameSize());
        spilledStatus.clear();
    ...
    private void processTuple(int tid, int pid) throws HyracksDataException {
        if (spilledStatus.get(tid)) {
            while (!bufferManager.insertTupleToSpilledPartition(pid, accessorBuild, tid, tempPtr)) {
    ...
    private void selectAndSpillVictim(int pid) throws HyracksDataException {
        int victimPartition = selectVictim(pid);
    ...
    private int selectVictim(int pid) {
        ...
        // try to flush the in memory partition.
        if (partitionToSpill < 0 || (maxToSpillPartSize = bufferManager.getPhysicalSize(partitionToSpill)) == ctx
                .getInitialFrameSize()) {
            int partitionInMem = selectLargestInMemoryPartition();
    ...
    private int selectLargestInMemoryPartition() {
        int pid = -1;
        int max = 0;
