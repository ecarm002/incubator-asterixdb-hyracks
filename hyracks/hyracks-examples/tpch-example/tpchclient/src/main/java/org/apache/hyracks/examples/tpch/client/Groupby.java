...
public class Groupby {
    private static class Options {
        ...
        @Option(name = "-algo", usage = "The algorithm to be used: hash|sort|inMemHash", required = true)
    ...
    public static void main(String[] args) throws Exception {
        ...
        job = createJob(parseFileSplits(options.inFileSplits), parseFileSplits(options.outFileSplits),
                options.htSize, options.frameLimit, options.frameSize, options.algo,
                options.outPlain);
        if (job != null) {
    ...
    private static JobSpecification createJob(FileSplit[] inSplits, FileSplit[] outSplits, int htSize, int frameLimit,
            int frameSize, String alg, boolean outPlain) {
        ...
        if (alg.equalsIgnoreCase("hash")) {// external hash graph
            grouper = new ExternalGroupOperatorDescriptor(spec, htSize,
                    keys, frameLimit, new IBinaryComparatorFactory[] {
                    ...
                    new MultiFieldsAggregatorFactory(
                            new IFieldAggregateDescriptorFactory[] { new IntSumFieldAggregatorFactory(keys.length,
                                    false) }), outDesc, outDesc, new HashSpillableTableFactory(
            ...
            grouper = new SortGroupByOperatorDescriptor(spec, frameLimit, keys, keys,
                    ...
                    new MultiFieldsAggregatorFactory(new IFieldAggregateDescriptorFactory[] {
                            new IntSumFieldAggregatorFactory(keys.length, true) }), outDesc, outDesc, false);

            createPartitionConstraint(spec, grouper, outSplits);

        } else if (alg.equalsIgnoreCase("inMemHash")) {
